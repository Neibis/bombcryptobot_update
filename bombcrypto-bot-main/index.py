...
def login(): #962 / 549
    ...
    if clickBtn(images['connect'], name='connectWalletBtn', timeout= 10):
        login_attempts = login_attempts + 1
        logger('🎉 Deu!')
...
def browser(imagem):
    ...
    x, y = pyautogui.locateCenterOnScreen((imagem + '.png'), confidence=.9)
...
def main():
    ...
    while True:
        ...
        if now - last["browser"] > addRandomness( t['refresh_heroes_positions'] * 30):
            last["browser"] = now
            try:
                browser('browser1')
